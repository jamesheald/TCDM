...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, synergies: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        distribution = self._get_action_dist_from_latent(latent_pi, synergies)
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, synergies: th.Tensor) -> Distribution:
        """
        ...
        if isinstance(self.action_dist, FullGaussianDistribution):
            return self.action_dist.proba_distribution(mean_actions, self.log_std, synergies)
    ...
    def _predict(self, observation: PyTorchObs, synergies: th.Tensor, deterministic: bool = False) -> th.Tensor:
        """
        Get the action according to the policy for a given observation.
        ...
        :return: Taken action according to the policy
        """
        return self.get_distribution(observation, synergies).get_actions(deterministic=deterministic)

    def evaluate_actions(self, obs: PyTorchObs, actions: th.Tensor, synergies: th.Tensor) -> tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
    ...
    def get_distribution(self, obs: PyTorchObs, synergies: th.Tensor) -> Distribution:
        """
        ...
        return self._get_action_dist_from_latent(latent_pi, synergies)
