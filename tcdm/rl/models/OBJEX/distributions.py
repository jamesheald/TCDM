...
class FullGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0, zlog_std_init: float = 0.0) -> Tuple[nn.Module, nn.Parameter]:
        """
        Create the layers and parameter that represent the distribution:
        one output will be the mean of the Gaussian, the other parameter will be the
        standard deviation (log std in fact to allow negative values)
        ...
        if self.state_dependent_std['diagonal']==False and self.state_dependent_std['low_rank']==False:
            mean_actions_and_log_std = nn.Linear(latent_dim, self.action_dim)
            # log_std = nn.Parameter(th.ones(self.action_dim + self.num_controlled_variables) * log_std_init, requires_grad=True)
            log_std = nn.Parameter(
                                        th.cat([
                                            th.full((self.action_dim,), log_std_init),
                                            th.full((self.num_controlled_variables,), zlog_std_init)
                                        ]),
                                        requires_grad=True
                                    )
            ...
            log_std = nn.Parameter(th.ones(self.num_controlled_variables) * zlog_std_init, requires_grad=True)
        else:
    ...
    def proba_distribution(self, mean_actions: th.Tensor, zlogstd: th.Tensor, log_std: th.Tensor, channel: th.Tensor, explore_cond: th.Tensor, log_std_init: float = 0.0, zlog_std_init: float = 0.0) -> "FullGaussianDistribution":
        """
        Create the distribution given its parameters (mean, std)
        ...
        if not self.standard_PPO:
            ...
            if explore_cond.sum() == 0:
                ...
                covariance_matrix[explore_cond] += low_rank[explore_cond]

                if self.use_gram_schmidt:

                    explore_dist = Independent(Normal(loc=th.zeros_like(explore_std[explore_cond]), scale=explore_std[explore_cond]), 1)
                    explore_entropy = explore_dist.entropy()

                else:

                    explore_entropy = entropy_low_rank_gaussian(intermediate[explore_cond])

                # explore_dist = MultivariateNormal(loc=th.zeros_like(zlogstd[explore_cond]), covariance_matrix=low_rank[explore_cond] + 1e-6 * th.eye(self.num_controlled_variables, device=channel.device).unsqueeze(0))

            self.distribution = MultivariateNormal(loc=mean_actions, covariance_matrix=covariance_matrix)

            if self.state_dependent_std['low_rank']:
                return self, action_std, th.where(explore_cond[:,None].expand_as(explore_std), explore_std, th.full(explore_std.shape, float('nan'), device=zlogstd.device, dtype=explore_std.dtype)), diagonal_entropy, explore_entropy
            else:
